...
fn main() {
    ...
    if args.len() != 5 {
        eprintln!("Usage {} FILE PIXELS UPPERLEFT LOWERERIGHT", args[0]);
        eprintln!(
...
///
/// Specially, `s` should have the form <left><sep><right>, where <sep> is
...
///
/// `bounds` is a pair given the width and height of the image in pixels.
...
/// which holds grayscale pixel per byte. The `upper_left` and `lower_right`
